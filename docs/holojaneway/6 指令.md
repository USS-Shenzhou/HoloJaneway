...
以`/excute as @p run yourcommand 10`为例，你直接使用`ct.getArgument("yourParameter", Integer.class)`只会吃到一个`No such argument`的大比兜子。使用`ct.getChild().getArgument("yourParameter", Integer.class)`才能正常地获得这个`10`。
