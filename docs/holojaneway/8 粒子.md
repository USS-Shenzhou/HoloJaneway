...
如果你想在粒子中实现比渲染出来多一点点的逻辑，都不要忘记考虑性能问题。单个粒子可能只需要很少时间就能完成`tick`，但当粒子数量达到一万个或是十万个时，其性能消耗就会变得非常可观。

以下有一些可行的办法：

- 在生成时大概估算好数量，尽可能控制复杂逻辑粒子的数量；
- 为粒子的每一个逻辑部分都增加开关，只在需要的时候开启，不需要是关闭——让CPU判断一万次`true`或`false`比算一万次碰撞快多了；
