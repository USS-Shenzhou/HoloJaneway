...
这篇教程主要在于帮助想学习mod开发的你入坑，但与其他教程可能略微不同的是，我包含了在开发《我的六号（Rainbow6:Minesiege）》（以下简称R6MS）、《灭 Extinguish》、《亮光 Brighter》、*T88*、《疯狂粒子 MadParticle》等模组时的经验总结。所以，不像通常的教程会教你写一个`foo`物品、`bar`方块，我更倾向于告诉你如何从无至有地写出破片手榴弹、消防水炮等等具体的物品、方块、实体或者其他的什么东西。
...
FML全称是Forge Mod Loader，它是一个mod加载器，能够让你一次把jar拖进`/mods`里就能装一大堆mod，而不是去用覆盖jar文件的方式。
...
用`ForgeGameTestHooks.isGametestEnabled()`就可以判断了。
